...
public class CameraFollow : MonoBehaviour
{
    ...
    {
        Vector3 tempTracker = new Vector3(playerTracker.position.x, playerTracker.position.y, -10);

        if(playerTracker.position.x < minX)
        {
            tempTracker = new Vector3(minX, playerTracker.position.y, -10);
        }else if(playerTracker.position.x > maxX)
        {
            tempTracker = new Vector3(maxX, playerTracker.position.y, -10);
        }

        if (playerTracker.position.y < minY)
        {
            tempTracker = new Vector3(tempTracker.x, minY, -10);
        }
        else if (playerTracker.position.y > maxY)
        {
            tempTracker = new Vector3(tempTracker.x, maxY, -10);
        }
